  ...
  template: `
    <div class="bg-gray-900 text-white font-sans min-h-screen">
      ...
      <div class="pt-24 px-4 max-w-7xl mx-auto space-y-12 pb-12 relative">
        ...
        <!-- Loading Overlay -->
        <div *ngIf="isLoadingData" class="absolute inset-0 z-40 bg-gray-900/50 backdrop-blur-sm flex items-start justify-center pt-20 animate-fade-in rounded-xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in" *ngIf="stats">
            ...
            <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg relative overflow-hidden">
                <div class="absolute top-0 right-0 p-4 opacity-10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                </div>
        ...
        <section *ngIf="pendingUsers.length > 0" class="space-y-4 animate-fade-in">
          ...
          <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div *ngFor="let user of pendingUsers" class="bg-gray-800 p-6 rounded-xl border border-yellow-500/30 shadow-lg hover:border-yellow-500/50 transition">
                <div class="flex justify-between items-start mb-4">
                    <div class="overflow-hidden">
                        <h3 class="font-bold text-lg text-white truncate" title="{{user.email}}">{{ user.email }}</h3>
                        <span class="text-xs text-gray-500 font-mono truncate block" title="{{user.uid}}">UID: {{ user.uid }}</span>
                    </div>
                    <span class="px-2 py-1 bg-yellow-900/50 text-yellow-200 text-xs rounded border border-yellow-700 flex-shrink-0">Pending</span>
                </div>
                
                <div class="space-y-2 text-sm text-gray-300 mb-6 bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                    <p class="flex flex-col"><strong class="text-gray-500 text-xs uppercase tracking-wide mb-1">Institution</strong> {{ user.onboarding?.institution }}</p>
                    <p class="flex flex-col"><strong class="text-gray-500 text-xs uppercase tracking-wide mb-1">Website</strong> <a [href]="user.onboarding?.institutionLink" target="_blank" class="text-blue-400 hover:underline truncate block">{{ user.onboarding?.institutionLink }}</a></p>
                    <p class="flex flex-col"><strong class="text-gray-500 text-xs uppercase tracking-wide mb-1">Why Soil?</strong> <span class="italic text-gray-400">"{{ user.onboarding?.explanation }}"</span></p>
                </div>

                <div class="flex gap-3">
                    <button (click)="initiateApprove(user)" class="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition shadow-lg">Approve</button>
                    <button (click)="rejectUser(user)" class="flex-1 py-2 bg-red-900/50 border border-red-700 hover:bg-red-900 text-red-200 font-bold rounded-lg transition">Reject</button>
                </div>
            </div>
          </div>
        </section>
                ...
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-gray-700 text-sm">
                    <tr *ngFor="let admin of admins" class="hover:bg-gray-700/30 transition group">
                      ...
                      <td class="p-4">
                        <div class="flex items-center gap-2">
                             <div class="w-full bg-gray-700 rounded-full h-1.5 w-16">
                                <div class="bg-blue-500 h-1.5 rounded-full" [style.width.%]="(admin.gameCount / admin.quota) * 100"></div>
                             </div>
                      ...
                      <td class="p-4 text-right flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition">
                        <ng-container *ngIf="admin.role !== 'superadmin'">
                            <button (click)="viewGames(admin)" class="px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-xs transition border border-gray-600">
                                Games
                            </button>
                            <button (click)="setQuota(admin)" class="px-2 py-1 bg-gray-700 hover:emerald-800 text-gray-300 rounded text-xs transition border border-gray-600" title="Edit Quota">
                                Quota
                            </button>
                            <button (click)="banAdmin(admin)" class="px-2 py-1 bg-gray-700 hover:bg-orange-900 text-orange-200 rounded text-xs transition border border-gray-600" title="Ban User">
                                Ban
                            </button>
                             <button (click)="deleteAdmin(admin)" class="px-2 py-1 bg-gray-700 hover:bg-red-900 text-red-200 rounded text-xs transition border border-gray-600" title="Delete User">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                             </button>
                        </ng-container>
                        <span *ngIf="admin.role === 'superadmin'" class="text-gray-600 italic text-xs px-2">Protected</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
        ...
        <section *ngIf="selectedAdmin" class="pt-8 border-t border-gray-800 animate-fade-in" id="games-section">
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-2xl font-bold text-white">Games by <span class="text-emerald-400">{{ selectedAdmin.email }}</span></h2>
                </div>
                <button (click)="selectedAdmin = null" class="text-gray-400 hover:text-white flex items-center gap-1 text-sm bg-gray-800 px-3 py-1.5 rounded transition hover:bg-gray-700">
            ...
            <div *ngIf="!isLoadingGames && adminGames.length === 0" class="p-12 bg-gray-800 rounded-xl text-center text-gray-500 border border-gray-700 border-dashed">
                <p class="text-lg">No games found for this admin.</p>
            </div>

            <div *ngIf="!isLoadingGames && adminGames.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 <div *ngFor="let game of adminGames" class="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-emerald-500/50 transition shadow-lg group relative">
                    <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition">
                         <button (click)="deleteGame(game)" class="text-red-500 hover:text-red-300 p-2 rounded hover:bg-red-900/30" title="Delete Game">
                    ...
                    <div class="text-sm text-gray-400 space-y-2 mb-4">
                        <div class="flex justify-between border-b border-gray-700 pb-2 mb-2">
                             ...
                             <span class="text-emerald-400 font-bold bg-emerald-900/20 px-2 py-0.5 rounded text-xs border border-emerald-900/50">{{ game.status | uppercase }}</span>
                        </div>
                        ...
                        <div class="flex justify-between">
                         ...
                         <div class="text-[10px] text-gray-600 font-mono mt-2 truncate bg-gray-900/50 p-1 rounded">{{ game.id }}</div>
                    </div>
                 </div>
            </div>
        </section>
        ...
        <div *ngIf="gameToDelete"
            class="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm"
            (click)="cancelDelete()">
            <div class="bg-gray-800 border border-gray-700 p-6 rounded-xl flex flex-col gap-4 max-w-md w-full shadow-2xl"
                (click)="$event.stopPropagation()">
                <h3 class="text-xl font-bold flex items-center gap-2" [class.text-red-400]="!isTrash(gameToDelete)" [class.text-red-600]="isTrash(gameToDelete)">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <p class="text-gray-300">
                    Are you sure you want to {{ isTrash(gameToDelete) ? 'permanently' : '' }} delete <span class="font-bold text-white">{{ gameToDelete.name }}</span>?
                </p>

                <!-- Soft Delete Info -->
                <div *ngIf="!isTrash(gameToDelete)" class="bg-red-900/20 border border-red-900/50 p-3 rounded-lg flex gap-3 items-start">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-400 flex-shrink-0 mt-0.5"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <div class="text-sm text-red-200/80">
                        <strong>Soft Delete:</strong> Games will be moved to the Trash.
                    </div>
                </div>

                <!-- Permanent Delete Warning -->
                <div *ngIf="isTrash(gameToDelete)" class="bg-red-600/20 border border-red-500/50 p-3 rounded-lg flex gap-3 items-start animate-pulse-slow">
                ...
                <div *ngIf="isTrash(gameToDelete)">
                     <label class="text-xs text-gray-400 block mb-1">Type <span class="font-mono font-bold text-red-400">DELETE</span> to confirm:</label>
                     <input [(ngModel)]="deleteConfirmInput" placeholder="DELETE" class="w-full px-4 py-2 bg-gray-900/50 border border-gray-600 rounded-lg text-white focus:border-red-500 focus:outline-none placeholder-gray-600">
                </div>
                ...
                <div class="flex gap-3 mt-4">
                    ...
                    <button (click)="confirmDelete()" [disabled]="isDeleting || (isTrash(gameToDelete) && deleteConfirmInput !== 'DELETE')"
        ...
        <div *ngIf="showQuotaModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
            ...
            <!-- Modal Content -->
            <div class="relative bg-gray-900 rounded-2xl shadow-2xl border border-gray-700 w-full max-w-md overflow-hidden transform transition-all scale-100">
                ...
                <div class="p-6 space-y-4">
                    <p class="text-gray-400 text-sm">
                        Set the maximum number of games allowed for <span class="text-white font-bold">{{ selectedUserForQuota?.email }}</span>.
                    </p>
                    
                    <div class="space-y-2">
                        <label class="block text-xs uppercase tracking-wide text-gray-500 font-bold">Quota Limit</label>
                        <input type="number" [(ngModel)]="newQuotaValue" min="0" 
        ...
        <div *ngIf="userToApprove" class="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
            ...
            <div class="relative bg-gray-800 border border-emerald-500/50 p-6 rounded-xl flex flex-col gap-4 max-w-sm w-full shadow-2xl" (click)="$event.stopPropagation()">
                ...
                <div class="flex gap-3 mt-2">
                    <button (click)="cancelApprove()" class="flex-1 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg font-medium transition">
                        Cancel
                    </button>
                    <button (click)="confirmApprove()" class="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition shadow-lg flex items-center justify-center gap-2">
                        Confirm Approval
                    </button>
                </div>
            </div>
