...
export class GameService {
    ...
    private pendingDecisions: { [index: number]: CropType } = {};
    ...
    async saveDraft(gameId: string) {
        ...
        const decision: RoundDecision = {
            machines: 0,
            organic: false, fertilizer: false, pesticide: false, organisms: false,
    ...
    async submitRound(gameId: string, settings?: { machines: number, organic: boolean, fertilizer: boolean, pesticide: boolean, organisms: boolean }) {
    ...
    async getAdminGames(page: number, pageSize: number, showDeleted: boolean = false, targetUid?: string): Promise<{ games: any[], total: number }> {
    ...
    async submitOnboarding(data: { firstName: string, lastName: string, explanation: string, institution: string, institutionLink?: string }) {
    ...
    async deleteGames(gameIds: string[], force: boolean = false): Promise<void> {
