...
describe('GameService', () => {
  ...
  it('should call saveDraft when updating parcel decision if gameId is provided', async () => {
    vi.useFakeTimers();
    const saveDraftMock = mockCallables.saveDraft;
  ...
  it('should load game data correctly', async () => {
    ...
    mockCallables.getGameState.mockResolvedValue({ data: mockData });

    const result = await service.loadGame('test-game-id');

    expect(result).toEqual(mockData);
    expect(mockCallables.getGameState).toHaveBeenCalledWith({ gameId: 'test-game-id' });
  });

  it('should fetch round data and cache it', async () => {
    const mockRound = { number: 1, parcelsSnapshot: [{ index: 0, crop: 'Wheat' }] };
    mockCallables.getRoundData.mockResolvedValue({ data: mockRound });

    const result = await service.getRoundData('test-game-id', 1);

    expect(result).toEqual(mockRound);
    expect(mockCallables.getRoundData).toHaveBeenCalledWith({ gameId: 'test-game-id', roundNumber: 1 });
  });

  it('should submit round correctly', async () => {
    const mockRound = { parcelsSnapshot: service.getParcelsValue() };
    mockCallables.submitDecision.mockResolvedValue({ data: { status: 'calculated', nextRound: mockRound } });
    ...
    expect(result).toEqual(mockRound);
    expect(mockCallables.submitDecision).toHaveBeenCalled();
  });

  it('should create game correctly', async () => {
    mockCallables.createGame.mockResolvedValue({ data: { gameId: 'new-game-id' } });
    ...
    expect(mockCallables.createGame).toHaveBeenCalled();
  });

  it('should delete games', async () => {
    await service.deleteGames(['game1', 'game2'], true);

    expect(mockCallables.deleteGames).toHaveBeenCalledWith({ gameIds: ['game1', 'game2'], force: true });
  });

  it('should fetch admin games', async () => {
    const mockGames = { games: [{ id: '1', createdAt: new Date() }], total: 1 };
    mockCallables.getAdminGames.mockResolvedValue({ data: mockGames });

    const result = await service.getAdminGames(1, 10, false);

    expect(result.games).toHaveLength(1);
    expect(result.total).toBe(1);
    expect(mockCallables.getAdminGames).toHaveBeenCalled();
  });

  it('should undelete games', async () => {
    await service.undeleteGames(['game1']);

    expect(mockCallables.undeleteGames).toHaveBeenCalledWith({ gameIds: ['game1'] });
  });

  it('should manage admin', async () => {
    await service.manageAdmin('user1', 'ban');

    expect(mockCallables.manageAdmin).toHaveBeenCalledWith({ targetUid: 'user1', action: 'ban', value: undefined });
  });

  it('should load local game if gameId starts with local-', async () => {
    const mockLocalState = {
      game: { id: 'local-123', currentRoundNumber: 0, status: 'in_progress' },
      playerState: { uid: 'p1' },
      allRounds: { p1: [] },
    };
  ...
  it('should update player type', async () => {
    ...
    expect(mockCallables.updatePlayerType).toHaveBeenCalledWith({
      gameId: 'game1',
      playerNumber: 1,
      type: 'ai',
      aiLevel: 'high',
  ...
  it('should update round deadline', async () => {
    ...
    expect(mockCallables.updateRoundDeadline).toHaveBeenCalledWith({
      gameId: 'game1',
      roundNumber: 2,
      deadline: '2026-01-01',
    });
  ...
  it('should submit feedback', async () => {
    ...
    expect(mockCallables.submitFeedback).toHaveBeenCalledWith(feedback);
  });

  it('should get all feedback', async () => {
    mockCallables.getAllFeedback.mockResolvedValue({ data: [] });
    await service.getAllFeedback();

    expect(mockCallables.getAllFeedback).toHaveBeenCalled();
  });

  it('should manage feedback', async () => {
    await service.manageFeedback('fb1', 'resolve');

    expect(mockCallables.manageFeedback).toHaveBeenCalledWith({
      feedbackId: 'fb1',
      action: 'resolve',
      value: undefined,
    });
  ...
  it('should export full game state', async () => {
    const mockRound = { number: 0, parcelsSnapshot: [] };
    mockCallables.getRoundData.mockResolvedValue({ data: mockRound });

    // Mock stateSubject value
    (service as any).stateSubject.next({
      game: { id: 'test', currentRoundNumber: 0, players: { p1: { uid: 'p1', history: [] } } },
    });

    await service.exportFullGameState('test');

    expect(mockCallables.getRoundData).toHaveBeenCalled();
  });
  ...
  it('should handle submit onboarding', async () => {
    ...
    expect(mockCallables.submitOnboarding).toHaveBeenCalledWith(onboardingData);
  });

  it('should handle errors in loadGame', async () => {
    mockCallables.getGameState.mockRejectedValue(new Error('Load failed'));

    await expect(service.loadGame('fail-id')).rejects.toThrow('Load failed');
  });

  it('should handle errors in submitDecision', async () => {
    mockCallables.submitDecision.mockRejectedValue(new Error('Submit failed'));
